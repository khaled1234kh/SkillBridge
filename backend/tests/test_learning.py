...
def test_learning_route_persists_item(client, student_id):
    student = models.get_student(student_id)
    gap = matching.gap_skills(student, student["target_role"])[0]
    r = client.post(f"/api/students/{student_id}/learning/generate", json={"skill_id": gap["skill_id"]})
...
def test_tutor_route_round_trip(client, student_id):
    r = client.post(f"/api/students/{student_id}/tutor", json={"message": "Help me with Docker"})
    assert r.status_code == 200
    assert r.json()["role"] == "assistant"
    assert r.json()["content"].strip()
    history = client.get(f"/api/students/{student_id}/tutor").json()
