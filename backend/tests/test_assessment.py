...
def test_high_severity_flag_forces_fail(client, student_id):
    ...
    r = client.post(f"/api/students/{student_id}/assessments/generate", json={"skill_id": docker["id"]})
...
def test_clean_pass_updates_verified_profile(client, student_id):
...
def test_fail_does_not_verify(client, student_id):
    # Wrong answers -> score below threshold -> no verification
    skill = models.create_skill("K8s", "DevOps")
    student = models.get_student(student_id)
    r = client.post(f"/api/students/{student_id}/assessments/generate", json={"skill_id": skill["id"]})
    questions = r.json()["questions"]
